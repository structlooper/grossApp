...
{{--                <p class="mb-0"><a class="text-primary" href="#"><i class="mdi mdi-web"></i> www.askbootstrap.com</a></p>--}}
                ...
                <ul>
                    @if($data['brands'])
                        @foreach($data['brands'] as $bnd)
                            @if(is_null($bnd->slug ))
                            @else
                            <li><a href="{{ route('brands',$bnd->slug ? $bnd->slug :'7up' ) }}">{{ str_replace('&amp;','&',$bnd->name) }}</a></li>
                            @endif
                        @endforeach
                    @else
            ...
            <div class="col-lg-2 col-md-2">
                ...
                <ul>
                    ...
                    <li><a href="{{ route('shop_main',$cat->slug) }}">{{ str_replace('&amp;','&',$cat->name) }}</a></li>
                        @endforeach
                    @else
            ...
            <div class="col-lg-2 col-md-2">
                ...
                <ul>
                    ...
                    <li><a href="{{route('about','privacy_policy')}}">Privacy policy</a></li>
                    <li><a href="{{route('about','return_refund')}}">Return policy</a></li>
        ...
        <div class="row no-gutters">
            <div class="col-lg-6 col-sm-6">
                <p class="mt-1 mb-0">&copy; Copyright 2020 <strong class="text-dark">{{ $data['site_settings']->site_name }}</strong>. All Rights Reserved<br>
                    <small class="mt-0 mb-0">Made with <i class="mdi mdi-heart text-danger"></i> by <a href="https://ssoftwares.in/" target="_blank" class="text-primary">Sikarwar Software</a>
                    </small>
                </p>
            </div>
...
<div class="cart-sidebar">
    ...
    <a class="btn btn-secondary btn-lg btn-block text-left" href="{{ route('checkout') }}" @if(Request::url() == url('user/checkout')) style="display:none;" @endif>
        <span class="float-left" ><i class="mdi mdi-cart-outline"></i> Proceed to Checkout </span>
        <span class="float-right" id="final_amount"></span><span class="mdi mdi-chevron-right"></span>
    </a>
</div>
                ...
                if (result.status == false){
                    ...
                    if (result.data.items == 0) {
                        ...
                        $.each(result.data.products, function (key, val) {
                            ...
                            finalTotal += (parseFloat(val.price)) * parseInt(val.qty)
                      ...
                      <div class="cart-list-product">
                        ...
                        <button class="btn btn-sm" onclick="remove_qty(${val.product_id})"><span class="mdi mdi-minus"></span></button>
                        <span style="font-size:15px;">${val.qty}</span>

                        <button class="btn btn-sm" onclick="add_qty(${val.product_id})"><span class="mdi mdi-plus"></span></button>
                    ...
                    `)
                      ...
                      <div class="cart-list-product">
                        ...
                        <button class="btn btn-sm" onclick="add_qty(${val.product_id})"><span class="mdi mdi-plus"></span></button>                        <p class="offer-price mb-0">${parseFloat(val.price).toFixed(2) * parseInt(val.qty)}₹<i class="mdi mdi-tag-outline"></i> <span class="regular-price">${parseFloat(val.cost).toFixed(2) * parseInt(val.qty)}₹</span></p>
                    </div>
                    `)
                            }
                        });
                    ...
                    `)
                        $('#final_amount').html(`<strong>${finalTotal} ₹</strong>`)
        ...
        $.ajax({
            ...
            success: function(result) {
                if (result.status == true) {
                    ...
                    if (result.msg == '3'){
                        alert('limited quantity available you can\'t add more of this item')
                        cart_data();
                    }else{
                        alert(result.msg)
                    }
        ...
        $('.add-cart-product').click(function (){
            ...
            @if(!is_null(Session::get('user_data')))
                ...
                $('.add-product-cart'+prod_id).html(`
                          <button class="btn btn-sm btn-secondary" onclick="remove_qty(${prod_id})"><span class="mdi mdi-minus"></span></button>
                            <span style="font-size:15px;" class="product-count${prod_id}">1</span>
                         <button class="btn btn-sm btn-secondary" onclick="add_qty(${prod_id})"><span class="mdi mdi-plus"></span></button>
...
`)
    ...
    $.get({
        url:geo_result,
        success(data){
            // console.log(data)"https://maps.googleapis.com/maps/api/geocode/json?key=AIzaSyA0ztr7esW-B9pyRKUYEa5d8CxLPnCdtdA&latlng="+lat+","+lng+""
...
<script>
    ...
    function initMap() {
        const map = new google.maps.Map(document.getElementById("map"), {
            center: { lat: 28.4796, lng: 77.0800 },
            zoom: 13,
        });
        ...
        autocomplete.addListener("place_changed", () => {
            ...
            var item_Location =place.address_components[0].long_name+', '+place.address_components[1].long_name+' '+place.address_components[2].long_name+' '+place.address_components[3].long_name;
            $("#lat").val(item_Lat);
            $("#lng").val(item_Lng);
...
<script async src="https://www.googletagmanager.com/gtag/js?id=UA-186056759-1">
</script>
<script>
